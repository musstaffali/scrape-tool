...
def query_amazon_books(keyword, date_filter=None):
    base_url = "https://www.amazon.com"
    query_url = f"{base_url}/s?k={keyword}"

    if date_filter:
        query_url += f"&rh=n%3A283155%2Cp_n_publication_date%3A{date_filter}"

    response = requests.get(query_url)
    soup = BeautifulSoup(response.content, "html.parser")

    book_listings = soup.select("[data-component-type='s-search-result']")
    book_titles = [listing.select_one("h2 a").text.strip() for listing in book_listings]
    return book_titles


def paginate_amazon_books(keyword, date_filter=None, max_pages=1):
    base_url = "https://www.amazon.com"
    query_url = f"{base_url}/s?k={keyword}"
    ...
    for page in range(1, max_pages + 1):
        ...
        book_listings = soup.select("[data-component-type='s-search-result']")
        titles = [listing.select_one("h2 a").text.strip() for listing in book_listings]
...
keyword = "fiction"
date_filter = "2023"  # Filter books published in 2023

# Query Amazon books by recent listings
book_titles = paginate_amazon_books(keyword, date_filter, max_pages=3)
print("Amazon Books:")
for title in book_titles:
    print(title)

# Get author contact information
contacts = get_author_contact(author_name)
print("Author Contacts:")
